...
import {
  AssignmentFetchTimeStorage,
  CurrentTime,
  MaxTimestamp,
  QuizFetchTimeStorage
} from "./constant"
...
export async function getEntities(
  ...
  const assignment: Array<Assignment> = await getAssignments(
    ...
    cacheOnly ||
      shouldUseCache(
        fetchTime.assignment,
        currentTime,
        settings.cacheInterval.assignment
      )
  )
  const quiz: Array<Quiz> = await getQuizzes(
    ...
    courses,
    cacheOnly ||
      shouldUseCache(fetchTime.quiz, currentTime, settings.cacheInterval.quiz)
  )
...
export async function getFetchTime(hostname: string): Promise<FetchTime> {
  const assignmentTime = await fromStorage<number | undefined>(
    hostname,
    AssignmentFetchTimeStorage,
    decodeTimestamp
  )
  const quizTime = await fromStorage<number | undefined>(
    hostname,
    QuizFetchTimeStorage,
    decodeTimestamp
  )
  ...
  return entries
    .filter((e) => {
      ...
      return (
        ...
        e.getTimestamp(appInfo.currentTime, option.showLateAcceptedEntry)
      )
    })
    .reduce(
      (prev, e) =>
...
export function createDateString(seconds: number | null | undefined): string {
  if (seconds === MaxTimestamp || seconds === undefined || seconds === null)
    return "----/--/--"
