...
const getSiteIdAndHrefSiteNameMap = (): Map<
  string,
...
> => {
  ...
  sites.forEach((site) => {
    const siteId = site
      .querySelector(".site-favorite-btn")
      ?.getAttribute("data-site-id")
    if (siteId == null) return
    const href = (
      site.querySelector(".fav-title")?.childNodes[1] as HTMLAnchorElement
    ).href
    const title = (
      site.querySelector(".fav-title")?.childNodes[1] as HTMLAnchorElement
    ).title
...
const getCurrentFavoritesSite = (): Array<string> => {
  ...
  for (const site of Array.from(sites)) {
    const href = (
      site.getElementsByClassName("link-container")[0] as HTMLAnchorElement
    ).href
    hrefs.push(href)
  }
  return hrefs
...
export const addFavoritedCourseSites = (baseURL: string): Promise<void> => {
  ...
  document
    .querySelector(".organizeFavorites")
    ?.addEventListener("click", editFavoritesMessage)
