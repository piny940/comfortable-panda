...
function MiniSakaiCourse(props: {
  ...
  const divClass = useMemo(
    () => `cs-assignment-${props.dueType}`,
    [props.dueType]
  )
  const aClass = useMemo(
    () => `cs-course-${props.dueType} cs-course-name`,
    [props.dueType]
  )
...
function MiniSakaiEntryList(props: {
  ...
  courseIdMap = new Map(
    [...courseIdMap.entries()].sort(sortCourseIdMap(props.settings))
  )
  ...
  for (const [courseID, entries] of courseIdMap.entries()) {
    ...
    courses.push(
      <MiniSakaiCourse
        ...
        coursePage={
          "https://" +
          props.settings.appInfo.hostname +
          "/portal/site/" +
          courseID
        }
        isSubset={props.isSubset}
...
const sortCourseIdMap = (settings: Settings) => {
  return function (
    ...
    const aMin = a[1].reduce(
      (prev, e) => Math.min(e.getDueDate(showLate), prev),
      MaxTimestamp
    )
