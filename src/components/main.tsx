...
export class MiniSakaiRoot extends React.Component<
  MiniSakaiRootProps,
  MiniSakaiRootState
> {
  ...
  reloadEntities() {
    ...
    getEntities(this.state.settings, getSakaiCourses(), cacheOnly).then(
      (entities) => {
        const allEntities = [
          ...entities.assignment,
          ...entities.quiz,
  ...
  private onMemoAdd(memo: MemoAddInfo) {
    ...
    saveNewMemoEntry(
      this.state.settings.appInfo.hostname,
      newMemo,
      memo.course
    ).then(() => {
      this.reloadEntities()
  ...
  componentDidUpdate(
    prevProps: MiniSakaiRootProps,
    prevState: MiniSakaiRootState
  ) {
...
function MiniSakaiAssignmentTime() {
  ...
  return (
    <MiniSakaiTimeBox clazz="cs-assignment-time" title={title} time={time} />
  )
}
