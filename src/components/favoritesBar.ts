...
const createCourseMap = (entities: EntityProtocol[]): CourseMap => {
  const courseMap = new Map<
    string,
    { entries: EntryProtocol[]; isRead: boolean }
  >()
  for (const entity of entities) {
    ...
    entries.isRead =
      entries.isRead && (entity.isRead || entity.entries.length === 0)
  }
  return courseMap
}
 ...
 */
export async function createFavoritesBar(
  settings: Settings,
  ...
  for (let j = 0; j < defaultTabCount; j++) {
    const aTag = defaultTab[j].getElementsByClassName("link-container")[0] as
      | HTMLAnchorElement
      | undefined
    const href = aTag?.href
    const hrefContent = href?.match(
      "(https?://[^/]+)/portal/site-?[a-z]*/([^/]+)"
    )
